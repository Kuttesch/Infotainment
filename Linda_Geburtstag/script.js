    ...
    .then(data => {
        ...
        if (SymbolIdTom in svgMap) {
            ...
            fetch(svgPath)
                .then(response => response.text())
                .then(svgData => {
                    const symbolTom = document.getElementById('symbol-tom');
                    symbolTom.innerHTML = svgData;
                    const svgElement = symbolTom.querySelector('svg');
                    svgElement.classList.add('svg-tom');
                });
        }
    });
}
